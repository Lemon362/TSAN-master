...
def outputStats_assess(KAPPA_AE, OA_AE, AA_AE, ELEMENT_ACC_AE, CATEGORY, path1, path2):
    ...
    sentence2 = 'AAs, mean_AA ± std_AA for each iteration are:' + str(AA_AE) + str(np.mean(AA_AE)) + ' ± ' + str(np.std(AA_AE)) + '\n'
    f.write(sentence2)
    # sentence3 = 'PRECISIONs for each iteration are:' + str(PRECISION) + '\n'
    # f.write(sentence3)
    # sentence4 = 'RECALLs for each iteration are:' + str(RECALL) + '\n'
    # f.write(sentence4)
    # sentence5 = 'F1SCOREs for each iteration are:' + str(F1SCORE) + '\n'
    # f.write(sentence5)
